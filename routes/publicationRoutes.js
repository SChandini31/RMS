  ...
  async (req, res) => {
    try {
      ...
      worksheet.columns = [
        ...
        { header: 'Date', key: 'date_of_publication', width: 18 },
      ...
      // log audit
      await AuditLog.create({
        action: 'download_publications_report',
        performedBy: req.user.id,
        role: req.user.role,
        department: currentUser?.department || '',
        school: currentUser?.school || '',
        targetType: 'report',
        targetId: null,
        details: `Downloaded publications Excel report (${publications.length} rows)`
      });

      res.end();
    } catch (error) {
      console.error('PUBLICATIONS EXCEL EXPORT ERROR:', error);
      res.status(500).json({ error: error.message });
    }
  }
);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res.setHeader('Content-Disposition', 'attachment; filename=publications.pdf');
      ...
      publications.forEach((p, idx) => {
        ...
        if (p.date_of_publication) doc.fontSize(10).text(`Date: ${new Date(p.date_of_publication).toLocaleDateString()}`);
        const authors = (p.authors || []).map(a => `${a.name || ''}${a.author_type ? ` (${a.author_type})` : ''}`);
        if (authors.length) doc.fontSize(10).text(`Authors: ${authors.join('; ')}`);
        doc.moveDown();
      });
      ...
      await AuditLog.create({
        ...
        targetType: 'report',
        targetId: null,
        details: `Downloaded publications PDF report (${publications.length} rows)`
      });

      doc.end();
    } catch (error) {
      console.error('PUBLICATIONS PDF EXPORT ERROR:', error);
      res.status(500).json({ error: error.message });
    }
  }
);
